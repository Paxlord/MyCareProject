...
import { Headline, Caption, Button, Checkbox, Paragraph, Subheading } from 'react-native-paper';
  ...
  return(
    <View style={{ flex: 8, paddingVertical: 8}}>
      ...
      <ScrollView style={{ flex: 1, marginVertical: 15 }}>
        {localTasks.map((task, index) => <Checkbox.Item label={task.label} style={{ backgroundColor: 'white', marginVertical: 5, marginHorizontal: 5, borderRadius: 5, elevation: 2 }} status={task.check?"checked":"unchecked"} disabled={true} color="#4DA29E"/>)}
      </ScrollView>
    </View>
  );
...
const VisitResume = ({route, navigation}) => {
  ...
  const DynamicButton = () => {
    switch(visitStatus){
      case 0:
        return  <Button onPress={() => updateStatus(1)}>Démarrer le rendez-vous</Button>
      case 1: 
        return <Button onPress={() => updateStatus(2)}>Terminer le rendez-vous</Button>
      case 2: 
        return <Button onPress={() => navigation.navigate("VisitsResume", {visitId: visitId})}>Laisser un résumé</Button>
    }
  }
  ...
  return(
    <SafeAreaView style={{ flex: 1 }}>
      ...
      {visitDetail.endTextResume && 
      <View style={{ flex: 3, backgroundColor: 'white', margin: 5, elevation: 2, borderRadius: 5, padding: 8}}>
       <Subheading style={{ color: '#4DA29E', fontWeight: 'bold'}}>Résumé de la visite</Subheading>
       <Paragraph>{visitDetail.endTextResume}</Paragraph>
      </View>}
    </SafeAreaView>
  );
}
